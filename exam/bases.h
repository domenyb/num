...
//    printf("Row_leap\n" );
    for (int j=0;j<m->cols;j++){
      printf("%g\t", extr_matrix_value(m, i,j));
    }
    printf("\n" );
  }
}
...
matrix* sqrt_diag_mtx(matrix* m){
  matrix * object=matrix_alloc(m->rows, m->cols);
  for (int i=0;i<m->cols;i++){//this function will only ever meet diagonal square matrices
    add_matrix_value(object, i, i, sqrt(extr_matrix_value(m, i, i)));
  }
  return object;
}

matrix* invert_diag_mtx(matrix* m){
  matrix * object=matrix_alloc(m->rows, m->cols);
  for (int i=0;i<m->cols;i++){//this function will only ever meet diagonal square matrices
    add_matrix_value(object, i, i, 1.0/(extr_matrix_value(m, i, i)));
  }
  return object;
}
...
void matrix_multiplication_2(matrix * m1, matrix * m2, matrix* object){//M1*M2
...
vector* eigen_vector(matrix* m, int number){
  vector* object=vector_alloc(m->rows);
  for (int i=0;i<object->n;i++){
    object->data[i]=extr_matrix_value(m, i, number);
  }
  return object;
}
